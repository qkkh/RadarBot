...
class RadarConfig:
    ...
    LOGS_ROOM_ID = 1498422633669197904 # روم اللوجات المطلوب
...
class RadarBot(commands.Bot):
    ...
    async def on_member_join(self, member):
        log_ch = self.get_channel(RadarConfig.LOGS_ROOM_ID)
        if log_ch:
            embed = discord.Embed(title="📥 عضو جديد", color=discord.Color.green(), timestamp=datetime.now())
            embed.add_field(name="العضو:", value=member.mention)
            embed.add_field(name="المعرف:", value=f"`{member.id}`")
            await log_ch.send(embed=embed)

    async def on_member_remove(self, member):
        log_ch = self.get_channel(RadarConfig.LOGS_ROOM_ID)
        if log_ch:
            embed = discord.Embed(title="📤 غادر السيرفر", color=discord.Color.dark_red(), timestamp=datetime.now())
            embed.description = f"العضو: {member.name}\nالمعرف: `{member.id}`"
            await log_ch.send(embed=embed)
