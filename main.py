...
# --- الإعدادات (تأكد من صحة الآيدي هات) ---
class RadarConfig:
...
# --- نظام الإحصائيات (تحديث الاسم فقط لتقليل اللاج) ---
async def refresh_radar_stats(guild):
    ...
    # الإحصائيات بالأسماء الجديدة اللي طلبتها
    stats_data = [
...
# --- نظام الترحيب بالصورة welcome.png ---
...
class TimeoutModal(discord.ui.Modal, title='تايم أوت ⏳'):
    u = discord.ui.TextInput(label="ID العضو")
    d = discord.ui.TextInput(label="المدة بالدقائق", default="10")
    async def on_submit(self, i):
        try:
            m = await i.guild.fetch_member(int(self.u.value))
...
class StreamModal(discord.ui.Modal, title='إشارة بث 📡'):
    ...
    async def on_submit(self, i):
        ...
        await i.response.send_message("✅ تم إطلاق إشارة البث", ephemeral=True)
...
class YoutubeModal(discord.ui.Modal, title='فيديو جديد 🎬'):
    ...
    async def on_submit(self, i):
        ...
        if ch: await ch.send(content=f"📣 @{self.m.value} فيديو جديد على القناة!\n{self.l.value}")
...
class SayModal(discord.ui.Modal, title='إرسال رسالة 📝'):
    ...
    async def on_submit(self, i):
        ...
        await i.response.send_message("✅ تم إرسال الرسالة", ephemeral=True)
...
class AdminDashboard(discord.ui.View):
    ...
    async def refresh_btn(self, i, b):
        ...
        await i.followup.send("✅ تم تحديث إحصائيات الرادار بنجاح", ephemeral=True)
...
class RadarBot(commands.Bot):
    ...
    async def on_ready(self): 
        ...
        print(f"📡 {self.user} is Online and Operating")

    @tasks.loop(minutes=10)
    async def auto_refresh_stats(self):
        for g in self.guilds:
            await refresh_radar_stats(g)
    ...
    async def on_member_join(self, member):
        ...
        if ch:
            ...
            if file: await ch.send(content=msg, file=file)
            else: await ch.send(content=msg)
    ...
    async def on_message_delete(self, m):
        ...
        if log:
            ...
            emb.add_field(name="الكاتب", value=m.author.mention)
            emb.add_field(name="المحتوى", value=m.content or "ملف/صورة")
...
# --- أوامر السلاش ---
@bot.tree.command(name="panel", description="فتح لوحة تحكم الرادار")
async def panel(i):
    if not has_radar_permission(i.user):
        return await i.response.send_message("❌ لا تملك الصلاحيات الكافية لاستخدام الرادار", ephemeral=True)
    
    emb = discord.Embed(title="🎮 RADARZ Dashboard", description="لوحة التحكم بجميع إشارات السيرفر متاحة الآن للإدارة المصرح لها", color=RadarConfig.MAIN_COLOR)
    
    try:
        # رفع صورة الداشبورد dashboard.png من الملفات المحلية
...
async def setup_dashboard(ctx):
    ...
    emb = discord.Embed(title="🎮 مركز تحكم الرادار الثابت", description="استخدم الأزرار أدناه لإدارة السيرفر والبثوث بشكل سريع", color=RadarConfig.MAIN_COLOR)
    try:
        file = discord.File("dashboard.png", filename="dashboard.png")
        emb.set_image(url="attachment://dashboard.png")
