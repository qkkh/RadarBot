...
class StreamModal(discord.ui.Modal, title='تجهيز إشارة البث الاحترافية 📡'):
    title_input = discord.ui.TextInput(label="عنوان البث", placeholder="مثلاً: مغامرة في ماينكرافت...", required=True)
    time_input = discord.ui.TextInput(label="بعد كم دقيقة يبدأ؟ (أرقام فقط)", placeholder="مثلاً: 1 أو 10", required=True)
    link_input = discord.ui.TextInput(label="رابط اليوتيوب", placeholder="ضع الرابط هنا لـسحب الغلاف...", required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # تحويل الدقائق إلى وقت Unix للعد التنازلي
        try:
            minutes = int(self.time_input.value)
            start_time = datetime.now() + timedelta(minutes=minutes)
            unix_timestamp = int(start_time.timestamp())
            countdown_tag = f"<t:{unix_timestamp}:R>" # عد تنازلي (مثلاً: بعد دقيقة)
            date_tag = f"<t:{unix_timestamp}:F>"      # التاريخ الكامل واليوم
        except ValueError:
            return await interaction.followup.send("⚠️ يرجى كتابة أرقام فقط في خانة التوقيت!", ephemeral=True)

        # استخراج صورة الثمنيل
        video_id = None
        regex = r"(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^\"&?\/\s]{11})"
        match = re.search(regex, self.link_input.value)
        thumbnail_url = f"https://img.youtube.com/vi/{match.group(1)}/maxresdefault.jpg" if match else "https://r2.community.sony.com/t5/image/serverpage/image-id/703649i6E2E195F919E3A9F/image-size/large?v=v2&px=999"
        ...
        embed.add_field(name="⏳ الانطلاق:", value=countdown_tag, inline=True)
        embed.add_field(name="🗓️ الموعد:", value=date_tag, inline=False)
        ...
        await interaction.followup.send("✅ تم إطلاق إشارة البث بنظام الوقت الحي!", ephemeral=True)

# (بقية الكلاسات Modal والـ View تبقى كما هي في الكود السابق)
...
async def panel(interaction: discord.Interaction):
    if interaction.user.id in RadarConfig.AUTHORIZED_USERS or interaction.user.guild_permissions.administrator:
        embed = discord.Embed(title="🎮 مركز عمليات RADARZ", description="تحكم في البث والإشعارات فوراً.", color=RadarConfig.MAIN_COLOR)
