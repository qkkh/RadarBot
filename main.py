...
# --- نظام أزرار الحضور والغياب (داخل الإعلان) ---
class StreamButtons(discord.ui.View):
    def __init__(self, stream_link):
        ...
        self.going = set()
        self.not_going = set()
        # إضافة زر الرابط كمكون مستقل ليعمل 100%
...
class StreamModal(discord.ui.Modal, title='تجهيز إشارة البث الاحترافية 📡'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.channel.send(content=content, embed=embed, view=StreamButtons(self.link_in.value))
...
class AdminDashboard(discord.ui.View):
    ...
    async def refresh_btn(self, i, b):
        await i.response.send_message("✅ جاري تحديث الإحصائيات (تأكد من ضبط ID الكاتيجوري)", ephemeral=True)
